...
### 2.8理论和定理
  ...
  5. 归纳偏置
     - 在机器学习中，很多学习算法image.png经常会对学习的问题做一些假设，这些假设就称为`归纳偏置`。

## 第3章 线性模型
  - 线性模型是机器学习中应用最广泛的模型，之通过样本特征的线性组合来进行预测的模型。给定一个$D$维样本$x = [x_1, x_2,…,x_D]^T$，其线性组合函数为
  $$f(x;w) = w_1 x_1 + w_2 x_2 + … + w_D x_D +b = w^Tx+b ,$$
  其中$w = [w_1, …,w_D]^T$为$D$维的权重向量，$b$是偏置。
  - 线性回归中直接用$f(x;w)$来预测$y$，即$y = f(x;w)$。
  - 在分类问题中，由于输出目标$y$是一些离散的标签，而$f(x;w)$的值域为实数，因此无法直接用$f(x;w)$来预测，需要引入一个非线性的`决策函数`$g(·)$来预测输出目标：
  $$y = g(f(x;w)),$$
  其中$f(x;w)$也称为`判别函数`。    
  对于二分类问题，$g(·)$可以是符号函数，定义为
  $$g(f(x;w)) = sgn(f(x;w)),$$
  当$f(x;w)$大于0时，$g(f(x;w))$为1, 当$f(x;w)$小于0时，$g(f(x;w))$为-1，当$f(x;w)$等于0时，不进行预测。其结构如下：
  ![二分类的线性模型](images/二分类的线性模型.png)

  ### 3.1 线性判别函数和决策边界
  1. **二分类**
     - 二分类问题的标签$y$只有两种取值，通常可以设为{+1，-1}，或者{0, 1}。
     - 在二分类问题中，只需要一个线性判别函数$f(x;w) = w^Tx+b$,特征空间$\Bbb R^D$中所有满足$f(x;w) = 0$的点组成一个分割`超平面`,称为`决策边界`或`决策平面`。决策边界将特征空间一分为二，划分成两个区域，每个区域对应一个类别。
     ![二分类的决策边界示例](./images/二分类的决策边界示例.png)
     - `两类线性可分`：对于训练集$\mathcal{D} = \{(x^{(n)},y^{(n)})\}_{n=1}^N$,如果存在权重向量$w^*$,对所有样本都满足$y f(x;w^*) > 0$,那么训练集$\mathcal{D}$可以被分为两个类别。
     - 为了学习参数$w$，我们需要定义合适的损失函数以及优化方法，对于二分类问题，最直接的损失函数为0-1损失函数。但0-1损失函数的数学性质不好，其关于$w$的导数为0，从而导致无法优化$w$.
  2. **多分类**
     - 多分类问题是指分类的类别数C超过2类。多分类一般需要多个线性判别函数。   
       假设一个多分类问题的类别为$\{1,2,3,…,C\}$，常用的方式有以下三种：
       1. ` “一对其余”方式`:把多分类问题转换成$C$个“一对其余”的二分类问题。这种方式需要$C$个判别函数，其中第$c$个判别函数$f_c$是将类别$c$的样本和不属于类别$c$的样本分开。
...
#### 3.4.1 参数学习
  - 感知器的学习算法是一种错误驱动的在线学习算法。先初始化一个权重向量$w \leftarrow 0(通常是全零向量)$，然后每次分错一个样本$(x,y)$时，即$y w^T x < 0$,就用这个样本来更新权重。
    $$ w \leftarrow w + yx$$
  - 感知器的损失函数为：
    $$\mathcal{L}(w; x, y) = \max(0, -y w^T x) $$
  - 感知器的更新过程：   
  ![感知器学习的更新过程](./images/感知器学习的更新过程.png)

#### 3.4.2 感知器的收敛性
  - `感知器收敛性`：给定训练集$\mathcal{D} = \{ x^{(n)}, y^{(n)} \}_{n=1}^N,$令$R$是训练集中最大的特征向量的模，即
    $$ R = \max_n \|x^{(n)}\| $$
    如果训练集$\mathcal{D}$线性可分，两类感知器的学习参数算法的权重更新频次数不超过$\frac{R^2}{\gamma^2}$sui
  - 虽然感知器在线性可分的数据上可以保证收敛，但存在以下不足：
    1. 在数据集线性可分时，感知器随谈可以找到一个超平面把数据分开，但并不能保证其泛化能力
    2. 感知器对样本顺序比较敏感，每次迭代的顺序不一致时，找到的分割超平面也往往不一致
    3. 如果训练集不是线性可分的，就永远不会收敛

#### 3.4.3 参数平均感知器
  - 如果训练数据是线性可分的，那么感知器可以找到一个判别函数来分割不同类的数据。如果间隔$\gamma$越大，收敛越快，但是感知器并不能保证找到的判别函数式最优的，这样可能导致过拟合。
  - 感知器学习到的权重向量和训练样本的顺序相关，在迭代次序上排在后面的错误样本比前面的错误样本对最终的权重向量影响更大。
  - 为了提高感知器的鲁棒性和泛化能力，我们可以将在感知器学习过程中的所有$K$个权重向量保存起来，并赋予每个权重向量$w_k$一个置信系数$c_k(1 \le c_k \le k \le K)$。最终的分类结果通过这$K$个不同权重的感知器投票决定，这个模型也称为`投票感知器`。
  - 令$\tau_k$为第$k$次更新权重$w_k$时的迭代次数(即训练过的样本数量)，$\tau_{k+1}$为下次权重更新时的迭代次数，则权重$w_k$的置信系数$c_k$设置为从$\tau_k$到$\tau_{k+1}$的之间间隔的迭代次数，即$c_k = {\tau_{k+1} - \tau_k}$，置信系数$c_k$的值越大，说明权重$w_k$在之后的训练过程中正确分类的样本数量越多，越值得信赖。       
  投票感知器的形式为：
  $$\hat y = sgn \bigg( \sum_{k=1}^K c_k sgn(w_k^T x) \bigg)$$
  - 投票感知器虽然提高了感知器的泛化能力，但是需要保存$K$个权重向量，在实际操作中会带来额外的开销。()
  - `平均感知器`通过使用“参数平均“的策略来减少投票感知器的参数数量。其形式如下：
    $$\hat y = sgn \bigg( \frac{1}{T} \sum_{k=1}^K c_k sgn(w_k^T x) \bigg) = sgn( \overline w^T x)$$
    其中$T$为迭代总回合数，$\overline w$为$T$次迭代的平均权重向量。

### 3.5 支持向量机
  - `支持向量机(SVM)`是一个经典的二分类算法。
  - 给定一个二分类器数据集，$\mathcal{D} = \{x^{(n)}, y^{(n)} \}_{n=1} ^ N$，其中$y_n \in \{-1, 1\}$，如果两类样本是线性可分的，即存在一个超平面$w^Tx + b = 0$将两类样本分开，那么对于每个样本都有$y^{(n)}(w^T x^{(n)} + b) > 0$。   
  数据集中每个样本$x^{(n)}$到分割超平面的距离为：
  $$\gamma^{(n)} = \frac{|w^T x^{(n)} +b|}{||w||} = \frac{y^{(n)} (w^T x^{(n)} +b)}{||w||}$$
  我们定义`间隔`$\gamma$ 为整个数据集中所有样本到分割超平面的最短距离：
  $$\gamma = \min_{n=1} \gamma^{(n)}$$
  如果间隔$\gamma$越大，其分割超平面对两个数据集的划分越稳定，不容易受噪声等因素影响。支持向量机的目标是寻找一个超平面使得$\gamma$最大。
  数据集中所有满足$y^{(n)} (w^T x^{(n)} +b) = 1 $的样本点，都称为`支持向量`。
  - 对于一个向量机，其分割超平面有很多个，但是间隔最大的超平面是唯一的，下图为最大间隔分割超平面的示例：
    ![支持向量机示例](./images/支持向量机示例.png)

#### 3.5.1 参数学习
#### 3.5.2 核函数
  - 支持向量机可以使用`核函数`隐式地将样本从原始特征空间映射到更高维的空间，并解决原始特征空间中的线性不可分问题。
#### 3.5.3 软间隔
  - 在支持向量机的优化问题中，约束条件比较严格，如果训练集中的样本在特征空间中不是线性可分的，就无法找到最优解，为了能够容忍部分不满足约束的样本，可以引入`松弛变量`，引入松弛变量的间隔称为`软间隔`。
  - 几种常见的线性模型对比   
    ![几种常见的线性模型对比](./images/几种常见的线性模型对比.png)


# 第二部分

## 第4章 前馈神经网络
 - 人工神经网路是指一系列受生物学和神经科学启发的数学模型，这些模型主要是通过对人脑的神经元网络进行抽象，构建人工神经元，并按照一定的拓扑结构来建立人工神经元之间的连接来模拟生物神经网络。
 -
