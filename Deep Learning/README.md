...
### 2.3 机器学习的简单示例————线性回归
  ...
  - 自变量为样本的特征向量$X\in R^D$(每一维对应一个自变量)，因变量是标签$y$。   
    假设空间是一组参数化的线性函数：
    $$f(x;w;b)=w^Tx+b$$  
    其中权重向量$w\in R^D$和偏置$b\inR $都是可学习的参数，函数$f(x;w;b)\inR$也称为`线性模型`
  - 参数估计方法：
    1. 经验风险最小
    2. 结构风险最小化
    3. 最大似然估计
...
### 2.6 数据的特征表示
   1. 传统的特征学习
      - `特征选择`：选取原始特征集合的一个有效子集，使得基于这个特征子集训练出来的模型准确率最高。简单地说，特征选择就是保留有用特征，移除冗余或无关的特征。
        ...
        1. **正则化**
      - `特征抽取`：是构造一个新的特征空间，并将原始特征投影在新的空间中得到新的表示。以线性投影为例，令$x\in R^D$为原始特征向量，$x' \in R^K$为经过线性投影后得到的在新空间中的特征向量，有
      $$x' = Wx$$ 
